...
train<-read.csv("H:\\data science\\data science\\hr analytics\\train.csv",header=T,sep=",",na.strings=c(""))
test<-read.csv("H:\\data science\\data science\\hr analytics\\test.csv",header=T,sep=",",na.strings=c(""))
summary(train)
str(train)
...
#_____________-bayes model__________

nb_model<-naiveBayes(is_promoted~department+region+education+gender+recruitment_channel+no_of_trainings+age+previous_year_rating+length_of_service+KPIs_met..80.+awards_won.+avg_training_score,data=train)
train$preds_nb<-predict(nb_model,train)
table(train$is_promoted,train$preds_nb)
summary(nb_model)
nb_model

#__________combination of models_____________

train$majority<-as.factor(ifelse(train$preds=="1"&train$preds_dt=="1","1",ifelse(train$preds_rf=="1"&train$preds_svm=="1","1",ifelse(train$preds=="1"&train$preds_svm=="1","1",0))))
table(train$majority,train$is_promoted)
