...
png(file="decision_tree_attrition.png")
model_tree<-ctree(Attrition~Age+DailyRate+Department+DistanceFromHome+
                    Education+EducationField+EnvironmentSatisfaction+Gender+
                    HourlyRate+JobInvolvement+JobLevel+JobRole+JobSatisfaction+
                    MaritalStatus+MonthlyIncome+MonthlyRate+NumCompaniesWorked+PercentSalaryHike+RelationshipSatisfaction
...
summary(model_tree)
model_tree
data$preds_dt<-predict(model_tree,data,type="response")
table(data$Attrition,data$preds_dt)

########decision tree
install.packages('randomForest')
library('randomForest')
model_rf<- randomForest (Attrition~Age+DailyRate+Department+DistanceFromHome+
                           Education+EducationField+EnvironmentSatisfaction+Gender+
                           HourlyRate+JobInvolvement+JobLevel+JobRole+JobSatisfaction+
                           MaritalStatus+MonthlyIncome+MonthlyRate+NumCompaniesWorked+PercentSalaryHike+RelationshipSatisfaction
                         +TotalWorkingYears+WorkLifeBalance+YearsAtCompany+YearsInCurrentRole+
                           YearsSinceLastPromotion+YearsWithCurrManager,data=data)
data$predrf<-predict(model_rf,data)
table(data$Attrition,data$predrf)
